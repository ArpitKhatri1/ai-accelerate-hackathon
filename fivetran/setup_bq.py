...
try:
    ...
    # Look for a .env in repo root or connector directory
    env_candidates = [
    ...
    # dotenv is optional; if unavailable, we rely on process env vars
    pass
...
class FivetranBigQueryDestinationSetup:
    ...
    def __init__(self, config_file_path: str = None):
        ...
        # Prefer environment variables; fall back to config file if present
        self.api_key = os.getenv("FIVETRAN_API_KEY") or self.config.get("fivetran_api_key") or self.config.get("fivetran", {}).get("api_key")
        self.api_secret = os.getenv("FIVETRAN_API_SECRET") or self.config.get("fivetran_api_secret") or self.config.get("fivetran", {}).get("api_secret")
        self.base_url = 'https://api.fivetran.com/v1'
        
        # Create base64 encoded credentials for Basic Auth
        if not self.api_key or not self.api_secret:
            raise Exception("Missing Fivetran credentials. Set FIVETRAN_API_KEY and FIVETRAN_API_SECRET in environment or configuration.json")
        ...
        **kwargs
        ...
        payload = {
            ...
            "config": {
                ...
                "data_set_location": dataset_location
            }
        }
    ...
    def list_groups(self, limit: int = 100) -> Optional[list]:
        """List all groups with basic pagination support."""
        groups: list = []
        cursor: Optional[str] = None
        while True:
            endpoint = f"groups?limit={limit}"
            if cursor:
    ...
    def get_or_create_group(self, group_name: str) -> Optional[Dict]:
        """Return existing group by name or create it if not found."""
        print(f"[INFO] Ensuring group exists: {group_name}")
        try:
            groups = self.list_groups() or []
            for grp in groups:
                if isinstance(grp, dict) and grp.get('name') == group_name:
                    print(f"[INFO] Reusing existing group: {group_name} ({grp.get('id')})")
                    return {"data": grp}
        except Exception as e:
            print(f"[WARNING] Failed to list groups, will attempt to create: {e}")
    ...
    def test_api_connection(self) -> Optional[Dict]:
        """Test the API connection by listing groups (auth-required endpoint)."""
        return self._make_api_request('GET', 'groups?limit=1')
...
def main():
    ...
    group_name = os.getenv("FIVETRAN_GROUP_NAME", "AI_Hackathon_Demo")  # Name for the group
    # Prefer env var for project id; fallback remains empty if not set
    project_id = os.getenv("BIG_QUERY_PROJECT_ID", "")  # Google Cloud Project ID
    dataset_location = os.getenv("BIG_QUERY_DATASET_LOCATION", "US")  # BigQuery dataset location
    region = os.getenv("FIVETRAN_REGION", "GCP_US_EAST4")  # Fivetran region
    ...
    try:
        ...
        group_response = destination_setup.get_or_create_group(group_name)
